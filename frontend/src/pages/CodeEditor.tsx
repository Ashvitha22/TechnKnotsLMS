 ...
 * - submit -> real backend judge (http://localhost:5000/api/judge)
 */

const JUDGE_API_URL = "http://localhost:5000/api/judge";
...
export default function CodeEditor() {
  ...
  const problem = React.useMemo(() => {
    ...
    return (problemsData as any[]).find(
  ...
  const publicTests = React.useMemo(() => {
    if (problem && Array.isArray((problem as any).publicTests))
      return (problem as any).publicTests;
  ...
  const [code, setCode] = useState<string>(() => {
    ...
    if (
      ...
      (problem as any).starterCode.javascript
    ) {
  ...
  // load "solved" state from localStorage when problem changes
  useEffect(() => {
    const raw = safeGet(solvedStorageKey());
    if (!raw) {
      setIsSolved(false);
      return;
    }
    try {
      ...
      if (Array.isArray(arr)) {
        setIsSolved(arr.includes(problemId));
      } else {
        setIsSolved(false);
      }
    } catch {
  ...
  function markProblemSolved(pid: string) {
    try {
      const raw = safeGet(solvedStorageKey());
      ...
      if (!list.includes(pid)) {
        list.push(pid);
        safeSet(solvedStorageKey(), JSON.stringify(list));
      }
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      problem &&
      (problem as any).starterCode &&
      (problem as any).starterCode[language]
    ) {
  ...
  async function runSingleTestRemoteViaPiston(
    lang: string,
    userCode: string,
    testCase: { input: any[] }
  ) {
    ...
    if (lang === "javascript") {
      const wrapper = `${userCode}\nconsole.log(JSON.stringify(${entry}(${JSON.stringify(
        testCase.input[0]
      )}, ${JSON.stringify(testCase.input[1])})));`;
      return await callPiston("javascript", wrapper, "main.js");
    }

    if (lang === "python") {
      const wrapper = `${userCode}\nimport json\nprint(json.dumps(${entry}(${JSON.stringify(
        testCase.input[0]
      )}, ${JSON.stringify(testCase.input[1])})))`;
    ...
    if (lang === "java") {
      const arrLiteral = JSON.stringify(testCase.input[0])
        .replace(/\[/g, "{")
        .replace(/\]/g, "}");
    ...
    return {
      ok: false,
      actual: null,
      error: `Remote run for ${lang} not implemented.`,
    };
  }
  ...
  async function callPiston(
    ...
    const idTimeout = window.setTimeout(() => controller.abort(), timeout);
    try {
      ...
      if (!resp.ok) {
        ...
        return {
          ok: false,
          actual: null,
          error: `Piston returned ${resp.status}: ${text}`,
        };
      }
      ...
      if (err.name === "AbortError")
        return { ok: false, actual: null, error: "Execution timed out" };
      return { ok: false, actual: null, error: String(err) };
    }
  }
  ...
  async function handleRunCode() {
    ...
    if (!testsToRun.length) {
      toast({
        ...
        description: "This problem has no public tests configured.",
        variant: "destructive",
      });
      setIsRunning(false);
      return;
  ...
  async function handleSubmit() {
    ...
    setTestResults([]);
    setLogs([]);
    addLog("Submitting solution to backend judge...");

    try {
      const resp = await fetch(JUDGE_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          code,
          language,
          tests: publicTests, // from problems.json
          entryFn: solutionFunctionName, // correct function name per language
        }),
      });

      if (!resp.ok) {
        const text = await resp.text();
        addLog(`Judge error: HTTP ${resp.status}`);
        toast({
          title: "Judge error",
          description: `Server responded with ${resp.status}`,
          variant: "destructive",
        });
        setIsRunning(false);
        return;
      ...
      addLog(`Judge finished: ${passedCount}/${results.length} passed.`);

      if (results.length > 0 && passedCount === results.length) {
        // all tests passed -> mark as solved
        markProblemSolved(problemId);
        setIsSolved(true);
        toast({
          title: "Submission Complete",
          description: "Accepted ✓ All tests passed. Problem solved!",
        });
      } else {
        toast({
          title: "Submission Complete",
      ...
      addLog(`Judge request failed: ${String(err)}`);
      toast({
        title: "Network error",
        description: "Could not reach judge backend.",
        variant: "destructive",
      });
    } finally {
      setIsRunning(false);
        ...
        <div className="w-1/2 border-r border-border overflow-y-auto p-6">
          <div className="flex items-center gap-2 mb-4">
            <h1 className="text-2xl font-bold">
              {problem ? (problem as any).title : "Problem"}
            </h1>
            <span className="text-xs bg-success/10 text-success px-2 py-1 rounded">
              {problem ? (problem as any).difficulty : "—"}
            </span>
            {isSolved && (
          ...
          <div className="prose max-w-none">
            <p>
              {problem && (problem as any).description
                ? (problem as any).description
                : "Problem description will appear here if it's defined in the JSON."}
            </p>
          </div>
              ...
              <div className="bg-black text-white text-xs p-2 rounded h-20 overflow-auto">
                {logs.length === 0 ? (
                  <div className="text-gray-400">
                    No logs yet. Run or submit to see output.
                  </div>
                ) : (
