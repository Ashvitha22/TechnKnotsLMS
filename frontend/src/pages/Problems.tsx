...
interface Problem {
  ...
  category: string;
  acceptance: number;
  completed: boolean; // we won't rely on this anymore, but keep it from JSON
  createdAt: string;
}

const Problems = () => {
  const allProblems: Problem[] = problemsData;

  // ✅ read solved ids from localStorage ("solvedProblems")
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(arr)) {
        ...
        setSolvedIds(arr.map((x) => String(x)));
      } else {
        setSolvedIds([]);
      }
    } catch {
  ...
  // derive topics & categories & tags set
  const allTopics = useMemo(
    () => Array.from(new Set(allProblems.flatMap((p) => p.topics))).sort(),
    [allProblems]
  );
  const allCategories = useMemo(
    () => Array.from(new Set(allProblems.map((p) => p.category))).sort(),
    [allProblems]
  );
      ...
      Array.from(
        new Set(
          allProblems.flatMap((p) =>
            p.tags && p.tags.length ? p.tags : p.topics
          )
        )
      ).sort(),
    [allProblems]
  ...
  const filteredProblems = useMemo(() => {
    const filtered = allProblems
      ...
      .filter(
        (p) => selectedTopic === "all" || p.topics.includes(selectedTopic) || (p.tags && p.tags.includes(selectedTopic))
      )
      .filter(
    ...
    filtered.sort((a, b) => {
      ...
      return a.id - b.id; // stable default (id ascending)
    });
  ...
  // ✅ stats use solvedIds instead of JSON "completed"
  const stats = useMemo(() => {
            ...
            {Array.from({ length: totalPages }).map((_, idx) => {
              ...
              return (
                <Button
                  key={pageNumber}
                  size="sm"
                  variant={pageNumber === pageSafe ? "default" : "ghost"}
